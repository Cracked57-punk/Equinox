...
| 2. Auth layer | Magic-link login (teams + admins), session guards (`requireTeam`, `requireAdmin`), backup codes, Tier 2/3 enforcement | ✅ Done |
| 3. Admin panel shell | Tier-gated navigation/layout — Tier 2 sees only question-pool section, Tier 3 sees everything | 🔶 Up next |
| 4. Round 1→2 handoff | Pre-loaded 35-team checklist UI, admin checks off 25 qualifiers, confirm-to-lock action | ⬜ Not started |
